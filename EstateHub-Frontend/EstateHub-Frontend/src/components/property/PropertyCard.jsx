...
function PropertyCard({ property, isFavorite = false, onFavorite }) {
  ...
  return (
    <article className="group flex h-full flex-col border border-[#D8CFB9] bg-white transition-all duration-300 hover:-translate-y-0.5 hover:border-[#AD8332] hover:shadow-[0_8px_24px_-12px_rgba(23,27,33,0.25)]">
      {/* ================================================= */}
      {/* IMAGE */}
      {/* ================================================= */}

      <div className="relative aspect-[4/3] w-full overflow-hidden bg-[#F2ECDF]">
        {imageUrl ? (
          <img
            src={imageUrl}
          ...
          <div className="flex h-full flex-col items-center justify-center text-[#8A806D]">
            <div className="flex h-14 w-14 items-center justify-center border border-[#D8CFB9] bg-white">
              <Building2 size={26} strokeWidth={1.25} className="text-[#8C6924]/70" />
            </div>
            <span className="mt-3 text-xs">No image available</span>
          </div>
        )}

        <div className="absolute inset-0 bg-gradient-to-t from-[#171B21]/25 via-transparent to-transparent opacity-0 transition duration-300 group-hover:opacity-100" />

        {/* Status */}
        {property.status && (
          <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 border border-[#D8CFB9] bg-white/95 px-2.5 py-1 text-[11px] font-medium text-[#201C15] backdrop-blur">
          ...
          <button
            ...
            className={`absolute right-3 top-3 flex h-9 w-9 items-center justify-center border backdrop-blur transition-all duration-200 ${
              isFavorite
                ...
                : "border-[#D8CFB9] bg-white/95 text-[#4A4436] hover:border-[#B3564B]/40 hover:text-[#B3564B]"
            }`}
          >
            <Heart size={16} strokeWidth={2} className={isFavorite ? "fill-[#B3564B]" : ""} />
          </button>
        )}

        {/* Property type */}
        {property.propertyType && (
          <span className="absolute bottom-3 left-3 border border-[#AD8332]/60 bg-[#171B21]/85 px-2.5 py-1 text-[11px] font-medium text-[#D8B876] backdrop-blur">
            {property.propertyType}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        {/* Title */}
        <Link to={detailPath} className="flex items-start justify-between gap-3">
        ...
        <div className="mt-2 flex items-center gap-1.5 text-sm text-[#6B6252]">
          <MapPin size={14} className="shrink-0 text-[#8C6924]" />
          ...
          <div className="mt-4 flex items-center gap-4 border-y border-[#EAE2CF] py-3 text-sm text-[#4A4436]">
            {property.bedrooms && (
              <span className="flex items-center gap-1.5">
                <BedDouble size={15} className="text-[#8A806D]" />
            ...
            {property.area && (
              <span className="flex items-center gap-1.5">
                <Ruler size={15} className="text-[#8A806D]" />
